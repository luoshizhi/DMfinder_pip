...
use lib("/ifshk7/BC_PS/zhengwenyuan/install/Graph-0.9704/lib");
use lib("/ifshk7/BC_PS/zhengwenyuan/install/Graph-Easy-0.76/lib");
...
for (my $i = 0; $i < scalar(@seg_record)-1; $i++)
{
			...
			for(my $k = 0; $k < scalar(@SV); $k++)
			{
				...
				#my $chr2_loc="";
				#if ($temp_rec[4] =~ /\[/){
				#	@vcf_rec=split(/\[/, $temp_rec[4]);
				#	@chr2_rec = split(/\:/, $vcf_rec[1]);
				#	$chr2 = $chr2_rec[0];
				#	$chr2_loc = $chr2_rec[1];
				#}
				#elsif ($temp_rec[4] =~ /\]/){
				...
				#}
				my $chr2 = $temp_rec[8];
				my $chr2_loc = $temp_rec[9];
				#print $chr2,$chr2_loc;
			#print "aaaaaaaaaaaaa";
				if(($temp_rec[3] eq $seg_rec[0] || $temp_rec[3] eq "chr".$seg_rec[0]) && abs($temp_rec[4] - $seg_rec[1]) <= $window)
				{
					#print "See if the link goes to another segment";
